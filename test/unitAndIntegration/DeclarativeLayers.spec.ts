...
const initializateDeclarativeLayers = () => {
  ...
  testTileLayer2 = declarativeLayers.getLayerReferences()
    .testTileLayer2 as leaflet.TileLayer
...
describe('declarative layers', () => {
  ...
  describe('access to layers refrences', () => {
    it('exposes references to added layers (visible or not)', () => {
      expect(
        Object.keys(declarativeLayers.getLayerReferences()).length,
      ).toEqual(layers.length)
      expect(
        declarativeLayers.getLayerReferences().testTileLayer1,
      ).toBeDefined()
      expect(
        declarativeLayers.getLayerReferences().testTileLayer2,
      ).toBeDefined()
      expect(
        declarativeLayers.getLayerReferences().testGeoJsonLayer1,
      ).toBeDefined()
      expect(
        declarativeLayers.getLayerReferences().testImageOverlay1,
      ).toBeDefined()
    })
  })
  describe('adding layers initially', () => {
    ...
    describe('tile layers', () => {
      ...
      it('passes the tile layer options', () => {
        expect(testTileLayer1.options.zIndex).toEqual(
          (layers[0] as dataTypes.ITilesMetadata).options.zIndex,
        )
        expect(testTileLayer2.options.maxZoom).toBe(
          (layers[1] as dataTypes.ITilesMetadata).options.maxZoom,
        )
      })
    })
    describe('GeoJson layers', () => {
      ...
      describe('popups', () => {
        ...
        it('should bind popups to a layer', () => {
          expect(testGeoJsonLayer1.bindPopup).toHaveBeenCalledWith(
            geojsonFeatureCollection.features[0].properties.popupContent,
          )
        })
    ...
    describe('ImageOverlay Layers', () => {
      ...
      it('should pass image overlay options to Leaflet', () => {
        expect(
          declarativeLayers.getLayerReferences().testImageOverlay1.options
            .attribution,
  ...
  describe('adding  and removing layers after initialization', () => {
    ...
    beforeEach(() => {
      ...
      testAddLayerReference = declarativeLayers.addLayer(
        newGeoJsonMetadataVisible,
      )
    ...
    describe('adding layers to the map and references post inititialization', () => {
      ...
      describe('including layer options', () => {
        it('should pass on tileLayer options to the Leaflet layer', () => {
          expect(
            (layerReferences.testNewTileLayerVisible as leaflet.TileLayer)
              .options.zIndex,
          ).toEqual(
            (newTileLayerVisible as dataTypes.ITilesMetadata).options.zIndex,
          )
          expect(
            (layerReferences.testNewTileLayerInvisible as leaflet.TileLayer)
              .options.maxZoom,
          ).toEqual(
            (newTileLayerInvisible as dataTypes.ITilesMetadata).options.maxZoom,
          )
        })
        it('should pass on GeoJSON options to the Leaflet layer', () => {
          expect(
            layerReferences.testNewGeoJsonLayerVisible.options.attribution,
