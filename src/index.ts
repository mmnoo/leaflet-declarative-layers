...
;((factory, window) => {
  ...
  if (typeof window !== 'undefined' && window.L) {
    ...
    const message: string = `The Leaflet reference is not available for the\
 Leaflet Environmental Layers plugin. Try reordering your script tags.`
  ...
  const leafletDeclarativeLayersPlugin = {
    ...
    addLayer: (layerMetadata: ILayerMetadata) =>
      declarativeLayersInstance.addLayer(layerMetadata),
    removeLayer: (layer: ILeafletLayer) =>
      declarativeLayersInstance.removeLayer(layer),
    toggleLayer: (layer: ILeafletLayer) =>
      declarativeLayersInstance.toggleLayer(layer),
  }
