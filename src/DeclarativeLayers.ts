...
export class DeclarativeLayers {
  ...
  public addLayer = (
    layerMetadata: dataTypes.ILayerMetadata,
  ): dataTypes.ILeafletLayer => {
    this.initializeLayer(layerMetadata)
  ...
  private initializeLayer = (layerMetadata: dataTypes.ILayerMetadata): void => {
    const layer: dataTypes.ILeafletLayer = this.createLeafletLayer(
      layerMetadata,
    )
  ...
  private createLeafletLayer = (
    ...
    if (dataTypes.isTilesType(layerMetadata)) {
      ...
      return this.initializeGeoJsonLayer(
        layerMetadata as dataTypes.IGeoJsonMetadata,
      )
    } else if (dataTypes.isImageOverlayType(layerMetadata)) {
      return this.initializeImageOverlayLayer(
        layerMetadata as dataTypes.IImageOverlayMetadata,
      )
    } else {
      throw new Error(
        `the data type for ${layerMetadata!.id} isnt currently supported`,
      )
    }
  }
  private addLayerToReferences = (
    id: string,
    layer: dataTypes.ILeafletLayer,
  ) => {
    this.layerReferences[id] = layer
  }
  private shouldBeVisibleInitially = ({
    visibleInitially,
    id,
  }: dataTypes.ILayerMetadata): boolean => {
  ...
  private initializeTileLayer = (layerMetadata: dataTypes.ITilesMetadata) => {
    return new this.suppliedLeafletReference.TileLayer(
      layerMetadata.url,
      layerMetadata.options,
    )
  }
  private initializeImageOverlayLayer = (
    layerMetadata: dataTypes.IImageOverlayMetadata,
  ) => {
    return new this.suppliedLeafletReference.ImageOverlay(
      layerMetadata.url,
      layerMetadata.bounds,
      layerMetadata.options,
    )
  }
  private initializeGeoJsonLayer = (
    layerMetadata: dataTypes.IGeoJsonMetadata,
  ) => {
    const options: leafletTypes.GeoJSONOptions = layerMetadata.options
      ? layerMetadata.options
      : {}
    const onEachFeatureOptions: leafletTypes.GeoJSONOptions = {
      onEachFeature: (
        feature: geoJson.Feature,
        layer: leafletTypes.GeoJSON,
      ): void => {
        if (options.onEachFeature) {
          options.onEachFeature(feature, layer)
        }
        if (layerMetadata.generateFeaturePopupContent) {
          const popupContent: leafletTypes.Content = layerMetadata.generateFeaturePopupContent(
            feature,
          )
    ...
    const mergedOptions: leafletTypes.GeoJSONOptions = {
      ...options,
      ...onEachFeatureOptions,
    }
    return new this.suppliedLeafletReference.GeoJSON(
      layerMetadata.data,
      mergedOptions,
    )
  }
